...
const minMovesToStables = (reindeer, stables) => {
    const minMoves = [];
    reindeer.forEach(reindeer => {
        const deltaMoves = stables.map(stable => Math.abs(reindeer - stable));
        const minMovement = Math.min(...deltaMoves);
        stables.splice(deltaMoves.indexOf(minMovement),1);
        minMoves.push(minMovement);
    });
    return minMoves.reduce((acc, curr) => acc + curr, 0);
};

checkPerformance(minMovesToStables([2, 6, 9], [3, 8, 5]));
